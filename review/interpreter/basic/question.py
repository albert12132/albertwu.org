...
concept_questions = [
    {'description': """Describe what a "REPL" is.""",
     'solution': """REPL stands for Read-Eval-Print-Loop. The first step, Read, inolves the tokenizer and the parser -- they take user input (a string) and convert it into data structures that are understood by the evaluator. The Evaluator converts those data structures into values, which are then Printed out to the screen. The loop then restarts the whole process."""
     },
    {'description': """Describe what a tokenizer does.""",
     'solution': """The tokenizer takes user input (a string) and breaks it up into tokens. This is an intermediate step in the Parser"""
     },
    {'description': """Explain the difference between a parser and an evaluator.""",
     'solution': """The parser converts a string of user input into an expression. The parser is not responsible for evaluating the expression -- as such, the parser checks if expressions are well-formed, but not if they actually evaluate to intelligible values (e.g. in SCheme, <tt>(3 + 2)</tt> is well-formed, but doesn't evaluate to a proper value).</p>
